...
**Live finding from #9 — RESOLVED:** the v22/4K master was **~-33 LUFS (≈19 dB under YouTube's -14)**. A naive loudnorm introduced ~107ms A/V drift (qc-audio caught it). The clean fix — 2-pass dynamic loudnorm + `aresample=48000` + trim to exact video duration (video stream copied) — is at `~/Desktop/FILMING-KIT/biz-20apy/BIZ-20APY-UPLOAD-4K-LOUD.mp4` and **passes qc-audio: -14.24 LUFS, 0 clip, 0.7ms drift**. Hardening this surfaced 3 qc-audio miscalibrations (summing downmix → phantom clipping; absolute hot-window; loudness-dependent tail-click) — all fixed (commit `55c16f4`) and the metrics are now loudness-robust.
