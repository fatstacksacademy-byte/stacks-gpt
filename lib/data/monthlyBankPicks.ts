    ...
    picks: [
      {
        ...
        takeaway:
          "Do this one first — before you open anything else, while your ChexSystems file is still clean. Joining is the easy part, and most write-ups make it sound harder than it is: even though FourLeaf (formerly Bethpage) is a New York credit union, anyone in the country can become a member just by opening a $5 savings share — no NY residency, employer group, association, or donation required, and that easy nationwide path survived the 2026 rebrand. The only hard cutoff is that you must be a U.S. citizen or permanent resident (green-card holder), with an SSN and a government photo ID. The real gate isn't membership, it's ChexSystems: approval hinges mostly on how many accounts you've opened in the last 1-2 months (zero or one is ideal), and there's no hard credit pull — only a Chex inquiry. One data-point-backed tip: FourLeaf often kicks the application back with a 'complete/resubmit' step and pulls ChexSystems a SECOND time, so a common move is to re-freeze your Chex file right after the first submission to block that second inquiry. Once you're in, the $350 only needs ONE qualifying $500+ direct deposit within 90 days — one paycheck and you're paid. The other $200 is a separate, two-year play: +$100 at 12 consecutive months and +$100 at 24, and if you miss a single month you forfeit both. Only real payroll/pension/gov deposits count (ACH pushes are hit-or-miss), so grab the easy $350, then decide on the $200 — it's a great low-effort hold if you can split your payroll (one paycheck/month here, the rest chasing other bonuses) or if you're parked in a Chex cooldown anyway. Note it's once-per-lifetime.",
      },
      {
