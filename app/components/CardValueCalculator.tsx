...
const label: React.CSSProperties = { fontSize: 11, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 600 }
const sectionTitle: React.CSSProperties = { fontSize: 13, fontWeight: 700, color: "#ffffff" }
...
export default function CardValueCalculator({
  ...
  if (!selected || !result) {
    return <div style={{ color: "#6b7280", padding: 24 }}>No cards available.</div>
  }
    ...
    <div>
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-start" }}>
        ...
        <div style={{ ...card, flex: "1 1 340px", minWidth: 300 }}>
          ...
          <div style={{ position: "relative", marginBottom: 22 }}>
            <input
              ...
              style={{ width: "100%", padding: "10px 12px", fontSize: 15, fontWeight: 500, border: "1px solid #2a2e38", borderRadius: 8, background: "#161922", color: "#ffffff", outline: "none", boxSizing: "border-box" }}
            />
            {q && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, marginTop: 4, background: "#161922", border: "1px solid #23262e", borderRadius: 8, boxShadow: "0 6px 20px rgba(0,0,0,0.10)", maxHeight: 280, overflowY: "auto", zIndex: 30 }}>
                {cardMatches.length === 0 ? (
                  <div style={{ padding: "10px 12px", fontSize: 13, color: "#6b7280" }}>No cards match “{cardQuery.trim()}”.</div>
                ) : cardMatches.map((c) => (
                  <button
                    key={c.id}
                    type="button"
                    onClick={() => { setCardId(c.id); setCardQuery("") }}
                    style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, width: "100%", textAlign: "left", padding: "9px 12px", fontSize: 14, background: c.id === cardId ? "rgba(13,150,104,0.14)" : "#161922", border: "none", borderBottom: "1px solid #23262e", cursor: "pointer", color: "#ffffff", fontFamily: "inherit" }}
                  >
                    <span style={{ fontWeight: 500 }}>{c.card_name}</span>
                    <span style={{ fontSize: 11, color: "#6b7280", textTransform: "capitalize", flexShrink: 0 }}>{c.issuer.replace(/-/g, " ")}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          {result.minSpend > 0 ? (
            <>
              <div style={{ fontSize: 12, color: "#9aa1ad", margin: "4px 0 12px", lineHeight: 1.5 }}>
                Earn the <strong style={{ color: "#ffffff" }}>{bonusLabel}</strong> bonus by spending{" "}
                <strong style={{ color: "#ffffff" }}>{money(result.minSpend)}</strong> in the first{" "}
                {result.spendMonths || 3} month{(result.spendMonths || 3) === 1 ? "" : "s"}. How much will you
                actually put on it in that window?
              </div>
              <div style={{ position: "relative", maxWidth: 200 }}>
                <span style={{ position: "absolute", left: 9, top: "50%", transform: "translateY(-50%)", color: "#6b7280", fontSize: 13 }}>$</span>
                <input
                  type="number"
                  inputMode="numeric"
                  value={subWindowSpend}
                  onChange={(e) => setSubWindowSpend(Math.max(0, Number(e.target.value.replace(/[^\d]/g, "")) || 0))}
                  style={{ width: "100%", padding: "9px 10px 9px 20px", fontSize: 15, border: "1px solid #2a2e38", borderRadius: 8, background: "#161922", color: "#ffffff", outline: "none", textAlign: "right", boxSizing: "border-box" }}
                />
              </div>
              <div style={{ marginTop: 10 }}>
                <div style={{ fontSize: 12, color: "#9aa1ad", marginBottom: 5 }}>
                  What will you mostly put that spend on? <span style={{ color: "#6b7280" }}>(sets how much it earns)</span>
                </div>
                <select
                  ...
                  style={{ width: "100%", maxWidth: 280, padding: "9px 10px", fontSize: 14, border: "1px solid #2a2e38", borderRadius: 8, background: "#161922", color: "#ffffff", outline: "none", boxSizing: "border-box" }}
                >
              ...
              <div
                style={{
                  ...
                  background: result.bonusEarned ? "rgba(13,150,104,0.14)" : "#1c160a",
                  border: `1px solid ${result.bonusEarned ? "rgba(13,150,104,0.5)" : "#4a3a16"}`,
                  color: result.bonusEarned ? "#34d399" : "#f59e0b",
                }}
              >
                {result.bonusEarned ? (
            ...
            <div style={{ fontSize: 12, color: "#9aa1ad", margin: "4px 0 4px", lineHeight: 1.5 }}>
              {result.signupBonusPotential > 0
                ? <>No minimum spend — the <strong style={{ color: "#ffffff" }}>{bonusLabel}</strong> bonus posts automatically.</>
                : <>This card has no current sign-up bonus.</>}
            </div>
          )}

          <div style={{ borderTop: "1px solid #23262e", margin: "20px 0 16px" }} />
            ...
            {displayedCats.map((c) => (
              <div key={c.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14, color: "#cdd2db", fontWeight: 500 }}>{c.label}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                  <div style={{ position: "relative" }}>
                    ...
                    <input
                      ...
                      style={{ width: 100, padding: "8px 10px 8px 20px", fontSize: 14, border: "1px solid #2a2e38", borderRadius: 8, background: "#161922", color: "#ffffff", outline: "none", textAlign: "right", boxSizing: "border-box" }}
                    />
                  </div>
                  <button
                    type="button"
                    ...
                    disabled={!!c.core}
                    style={{ width: 22, height: 22, lineHeight: "20px", textAlign: "center", border: "none", background: "transparent", color: c.core ? "#2a2e38" : "#6b7280", cursor: c.core ? "default" : "pointer", fontSize: 16, fontFamily: "inherit", flexShrink: 0 }}
                  >
                    ×
                  </button>
                </div>
        ...
        <div style={{ flex: "1 1 340px", minWidth: 300, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <div style={{ ...card, flex: "1 1 140px", background: ACCENT_BG, border: "none" }}>
            ...
            <div style={{ ...card, flex: "1 1 140px" }}>
              ...
              <div style={{ fontSize: 38, fontWeight: 800, color: "#ffffff", lineHeight: 1.1, marginTop: 4 }}>{money(result.year2)}</div>
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 4 }}>rewards + recurring credits − fee</div>
            </div>
          </div>
          ...
          <div style={card}>
            ...
            <div style={{ borderTop: "1px solid #23262e", marginTop: 8, paddingTop: 10, display: "flex", justifyContent: "space-between", fontWeight: 800, fontSize: 16, color: "#ffffff" }}>
          ...
          <div style={card}>
            ...
            <div style={{ borderTop: "1px solid #23262e", marginTop: 8, paddingTop: 10, display: "flex", justifyContent: "space-between", fontWeight: 700, fontSize: 14, color: "#ffffff" }}>
      ...
      <div style={{ ...card, marginTop: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 8 }}>
          ...
          {result.hasIntroApr && (
            <span style={{ fontSize: 12, color: "#9aa1ad" }}>
              {selected.card_name} has 0% on purchases for <strong style={{ color: "#ffffff" }}>{result.promoMonths} months</strong>
            </span>
          )}
        </div>
        ...
        {!result.hasIntroApr ? (
          ...
          <>
            <div style={{ fontSize: 12, color: "#9aa1ad", margin: "6px 0 16px", lineHeight: 1.55, maxWidth: 760 }}>
            ...
            <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-start" }}>
              ...
              <div style={{ flex: "1 1 360px", minWidth: 300 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  ...
                  <button
                    ...
                    style={{ fontSize: 12, color: "#60a5fa", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "inherit" }}
                  >
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(96px, 1fr))", gap: 8 }}>
                  {aprSchedule.map((amt, i) => (
                    <div key={i}>
                      <div style={{ fontSize: 10, color: "#6b7280", marginBottom: 3, fontWeight: 600 }}>Mo {i + 1}</div>
                      <div style={{ position: "relative" }}>
                        ...
                        <input
                          ...
                          style={{ width: "100%", padding: "7px 8px 7px 18px", fontSize: 13, border: "1px solid #2a2e38", borderRadius: 7, background: "#161922", color: "#ffffff", outline: "none", textAlign: "right", boxSizing: "border-box" }}
                        />
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 14 }}>
                  <span style={{ fontSize: 13, color: "#9aa1ad" }}>HYSA APY</span>
                  <div style={{ position: "relative" }}>
                    <input
                      type="number"
                      step="0.1"
                      ...
                      style={{ width: 80, padding: "6px 22px 6px 10px", fontSize: 13, border: "1px solid #2a2e38", borderRadius: 8, background: "#161922", color: "#ffffff", outline: "none", textAlign: "right", boxSizing: "border-box" }}
                    />
                    <span style={{ position: "absolute", right: 9, top: "50%", transform: "translateY(-50%)", color: "#6b7280", fontSize: 13 }}>%</span>
                  </div>
                  <a href="/stacksos/intro-apr" style={{ fontSize: 12, color: "#60a5fa", textDecoration: "none", marginLeft: "auto" }}>
                    Full 0% strategy →
                  </a>
                </div>
              </div>
              ...
              <div style={{ flex: "1 1 240px", minWidth: 240 }}>
                <div style={{ background: "rgba(13,150,104,0.14)", border: `2px solid ${ACCENT}`, borderRadius: 12, padding: "18px 20px" }}>
                  ...
                  {aprDetail && (
                    <div style={{ fontSize: 12, color: "#9aa1ad", marginTop: 6, lineHeight: 1.5 }}>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", marginTop: 14 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#ffffff" }}>Add this float to Year 1 value</span>
                </label>
            ...
            {aprDetail && aprDetail.schedule.length > 0 && aprDetail.totalSpend > 0 && (
              <div style={{ overflowX: "auto", marginTop: 18 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid #23262e", textAlign: "left", color: "#6b7280" }}>
...
function Row({ title, sub, value, negative, accent, muted }: { title: string; sub?: string; value: string; negative?: boolean; accent?: boolean; muted?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "5px 0" }}>
      <div>
        <span style={{ fontSize: 14, color: muted ? "#9aa1ad" : "#cdd2db", fontWeight: 500 }}>{title}</span>
        {sub && <span style={{ fontSize: 11, color: "#6b7280", marginLeft: 8 }}>{sub}</span>}
      </div>
      <span style={{ fontSize: 14, fontWeight: 600, color: negative ? "#f87171" : accent ? ACCENT : "#ffffff", whiteSpace: "nowrap" }}>{value}</span>
    </div>
  )
}
