...
export default function RoadmapClient({ userEmail, userId, isPaid }: { userEmail: string; userId: string; isPaid: boolean }) {
  ...
  // Pay Profile saves automatically (debounced), but an explicit Save button
  // + confirmation makes the action feel intentional. The button flushes the
  // current profile immediately so it's guaranteed written when clicked.
  ...
  async function handleSaveProfile() {
    ...
    try {
      await upsertProfileClient(profile)
      setProfileSaveState("saved")
      track("pay_profile_saved", {})
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }} className="rm-content">
          ...
          <div style={{ background: DK.panel, border: `1px solid ${DK.border}`, borderRadius: 12, padding: "24px 28px", marginBottom: 28 }}>
            ...
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${DK.border}`, display: "flex", alignItems: "center", gap: 12 }}>
              <button
                ...
                style={{
                  ...confirmBtnLight,
                  cursor: profileSaveState === "saving" ? "wait" : "pointer",
                  opacity: profileSaveState === "saving" ? 0.6 : 1,
                }}
              >
        ...
        {!isPaid && (() => {
          ...
          const yearPotential = [...available]
            ...
            .slice(0, 12)
            .reduce((s, b) => s + (b.bonus.bonus_amount ?? 0), 0)
          return (
