...
function InfoRow({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid #f0f0f0" }}>
      ...
      <span style={{ fontSize: 14, fontWeight: 600, color: accent ? "#0d7c5f" : "#111" }}>{value}</span>
    </div>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 36 }}>
      <h2 style={{ fontSize: 20, fontWeight: 700, color: "#111", margin: "0 0 16px", letterSpacing: "-0.01em" }}>{title}</h2>
      {children}
    </div>
  )
}
          ...
          <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 12, padding: "16px" }}>
            {content.pros.map((pro, i) => (
              <div key={i} style={{ display: "flex", gap: 10, marginBottom: i < content.pros.length - 1 ? 10 : 0, alignItems: "flex-start" }}>
                <span style={{ color: "#0d7c5f", fontSize: 13, flexShrink: 0, marginTop: 2 }}>+</span>
                <span style={{ fontSize: 13, color: "#666", lineHeight: 1.5 }}>{pro}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 12, padding: "16px" }}>
            {content.cons.map((con, i) => (
              <div key={i} style={{ display: "flex", gap: 10, marginBottom: i < content.cons.length - 1 ? 10 : 0, alignItems: "flex-start" }}>
                ...
                <span style={{ fontSize: 13, color: "#666", lineHeight: 1.5 }}>{con}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {links.length > 1 && (
            <div style={{ marginTop: 16 }}>
              ...
              {links.slice(1).map((link: string, i: number) => (
                <div key={i}>
                  <a href={link} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: "#0d7c5f", textDecoration: "none", wordBreak: "break-all" }}>
            ...
            return (
              <div key={i} style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 12, padding: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>{money(tier.min_deposit)} deposit</span>
  ...
  return (
    <>
      ...
      <article style={{ maxWidth: 700, margin: "0 auto", padding: "48px 24px 60px" }}>
        ...
        {/* Title */}
        <h1 style={{ fontSize: 36, fontWeight: 800, color: "#111", letterSpacing: "-0.03em", margin: "0 0 12px", lineHeight: 1.15 }}>
          {post.title}
        </h1>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ fontSize: 13, color: "#999" }}>
            By <a href={YT} target="_blank" rel="noopener noreferrer" style={{ color: "#0d7c5f", textDecoration: "none", fontWeight: 600 }}>Nathaniel Booth</a>
          </span>
          <span style={{ fontSize: 13, color: "#bbb" }}>{post.date}</span>
          <span style={{ fontSize: 13, color: "#0d7c5f", fontWeight: 600 }}>{post.category}</span>
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 32, flexWrap: "wrap" }}>
          {post.tags.map(tag => (
            <span key={tag} style={{ fontSize: 11, color: "#888", background: "#f0f0f0", padding: "3px 10px", borderRadius: 99 }}>
              {tag}
            </span>
          ))}
        </div>
        ...
        <div style={{ marginTop: 20, padding: "24px", background: "rgba(255,0,0,0.03)", border: "1px solid rgba(255,0,0,0.1)", borderRadius: 12 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#111", marginBottom: 8 }}>Watch the Video Breakdown</div>
        ...
        <div style={{ marginTop: 32, display: "flex", gap: 20 }}>
          <Link href="/blog" style={{ fontSize: 14, color: "#0d7c5f", textDecoration: "none", fontWeight: 600 }}>
            &larr; All reviews
          </Link>
