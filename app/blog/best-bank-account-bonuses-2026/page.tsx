  ...
  return (
    <>
      ...
      <style>{`
        ...
        .hub-table th { text-align: left; padding: 10px 12px; font-size: 12px; color: #888; text-transform: uppercase; letter-spacing: 0.04em; border-bottom: 2px solid #222; }
        .hub-table td { padding: 12px; border-bottom: 1px solid #1a1a1a; font-size: 14px; }
        .hub-table tr:hover { background: rgba(136,224,109,0.03); }
      ...
      <header style={{ borderBottom: "1px solid #1a1a1a", padding: "16px 0" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <nav style={{ display: "flex", gap: 24, alignItems: "center" }}>
            <Link href="/blog/best-bank-account-bonuses-2026" style={{ fontSize: 13, color: "#88e06d", textDecoration: "none", fontWeight: 600 }}>All Bonuses</Link>
      ...
      <main style={{ maxWidth: 1000, margin: "0 auto", padding: "48px 24px 60px" }}>
        ...
        <p style={{ fontSize: 13, color: "#555", marginBottom: 8 }}>
          By <a href={YT} target="_blank" rel="noopener noreferrer" style={{ color: "#88e06d", textDecoration: "none" }}>Nathaniel Booth</a> | Last updated April 10, 2026
        </p>

        <p style={{ fontSize: 16, color: "#ccc", lineHeight: 1.7, margin: "16px 0", maxWidth: 700 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, margin: "24px 0 40px" }}>
          {[
            ...
            <div key={i} style={{ background: "#111", border: "1px solid #222", borderRadius: 10, padding: "16px", textAlign: "center" }}>
              <div style={{ fontSize: 24, fontWeight: 800, color: "#88e06d" }}>{s.value}</div>
        ...
        {/* Table of Contents */}
        <div style={{ background: "#111", border: "1px solid #222", borderRadius: 12, padding: "20px 24px", marginBottom: 40 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#fff", marginBottom: 12 }}>Table of Contents</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <a href="#checking-bonuses" style={{ fontSize: 14, color: "#88e06d", textDecoration: "none" }}>1. Best Checking Account Bonuses ({checkingSorted.length} offers)</a>
            <a href="#savings-bonuses" style={{ fontSize: 14, color: "#88e06d", textDecoration: "none" }}>2. Best Savings Account Bonuses ({savingsSorted.length} offers)</a>
            <a href="#how-it-works" style={{ fontSize: 14, color: "#88e06d", textDecoration: "none" }}>3. How Bank Bonuses Work</a>
            <a href="#getting-started" style={{ fontSize: 14, color: "#88e06d", textDecoration: "none" }}>4. How to Get Started</a>
            <a href="#faq" style={{ fontSize: 14, color: "#88e06d", textDecoration: "none" }}>5. Frequently Asked Questions</a>
          </div>
        </div>

        {/* ── CHECKING TABLE ── */}
        <div id="checking-bonuses" style={{ marginBottom: 48 }}>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#fff", margin: "0 0 8px" }}>Best Checking Account Bonuses</h2>
            ...
            <table className="hub-table">
              ...
              <tbody>
                {checkingSorted.map(({ post, bonus }, i) => {
                  ...
                  return (
                    <tr key={post.slug}>
                      <td style={{ color: i < 3 ? "#88e06d" : "#555", fontWeight: 700 }}>{i + 1}</td>
                      <td>
                        <Link href={`/blog/${post.slug}`} style={{ color: "#fff", textDecoration: "none", fontWeight: 600 }}>
                          {b.bank_name?.split("(")[0].trim()}
                        </Link>
                      </td>
                      <td style={{ color: "#88e06d", fontWeight: 700 }}>{money(b.bonus_amount)}</td>
                      <td style={{ color: "#bbb" }}>{req.min_direct_deposit_total ? money(req.min_direct_deposit_total) : "Yes"}</td>
                      <td style={{ color: "#999" }}>{req.deposit_window_days ? `${req.deposit_window_days}d` : "—"}</td>
                      <td style={{ color: fees.monthly_fee === 0 ? "#88e06d" : "#ff6b6b" }}>{fees.monthly_fee === 0 ? "$0" : fees.monthly_fee != null ? `$${fees.monthly_fee}` : "—"}</td>
                      <td><Link href={`/blog/${post.slug}`} style={{ fontSize: 12, color: "#88e06d", textDecoration: "none" }}>Review</Link></td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        <div id="savings-bonuses" style={{ marginBottom: 48 }}>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#fff", margin: "0 0 8px" }}>Best Savings Account Bonuses</h2>
          <p style={{ fontSize: 14, color: "#888", margin: "0 0 20px" }}>
                ...
                {savingsSorted.map(({ post, bonus }, i) => {
                  ...
                  return (
                    <tr key={post.slug}>
                      ...
                      <td style={{ color: "#88e06d", fontWeight: 700 }}>{money(t.bonus_amount)}</td>
                      <td style={{ color: "#bbb" }}>{money(t.min_deposit)}</td>
                      <td style={{ color: "#999" }}>{b.total_hold_days}d</td>
                      <td style={{ color: "#999" }}>{(b.base_apy * 100).toFixed(2)}%</td>
                      <td style={{ color: "#88e06d", fontWeight: 700 }}>{effApy}%</td>
                      <td><Link href={`/blog/${post.slug}`} style={{ fontSize: 12, color: "#88e06d", textDecoration: "none" }}>Review</Link></td>
                    </tr>
                  )
                })}
        ...
        <div id="how-it-works" style={{ marginBottom: 48 }}>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#fff", margin: "0 0 16px" }}>How Bank Bonuses Work</h2>
          <div style={{ fontSize: 15, color: "#bbb", lineHeight: 1.8 }}>
            ...
            <p style={{ margin: "0 0 16px" }}>
              <strong style={{ color: "#fff" }}>Checking bonuses</strong> use your direct deposit (paycheck). You temporarily route your paycheck to a new bank, meet the deposit requirement in 60-90 days, collect the bonus, and move to the next bank. Your paycheck does the work — no extra money needed.
            </p>
            <p style={{ margin: "0 0 16px" }}>
              <strong style={{ color: "#fff" }}>Savings bonuses</strong> require parking a lump sum (typically $15,000-$100,000) for 90-180 days. The bank pays a cash bonus on top of the base interest rate. The key metric is <strong style={{ color: "#88e06d" }}>effective APY</strong> — the total return (bonus + interest) annualized over the holding period.
            </p>
            <p style={{ margin: "0 0 16px" }}>
              <strong style={{ color: "#fff" }}>Most bonuses don{"'"}t affect your credit score.</strong> Banks typically perform a soft pull or ChexSystems inquiry when you open a checking/savings account. Neither impacts your FICO score. We flag the rare exceptions in each review.
            </p>
            <p style={{ margin: 0 }}>
              <strong style={{ color: "#fff" }}>Bank bonuses are taxable income.</strong> Banks report bonuses over $10 on a 1099-INT or 1099-MISC form. You{"'"}ll owe income tax on the bonus amount at your marginal rate. A $400 bonus might net you $280-$340 after taxes, depending on your bracket.
            </p>
          </div>
        </div>
        ...
        <div id="getting-started" style={{ marginBottom: 48 }}>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#fff", margin: "0 0 16px" }}>How to Get Started</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {[
              ...
              <div key={i} style={{ display: "flex", gap: 16, background: "#111", border: "1px solid #222", borderRadius: 12, padding: "20px" }}>
                <div style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(136,224,109,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: "#88e06d" }}>{item.step}</span>
                </div>
                <div>
        ...
        {/* ── STACKS OS CTA ── */}
        <div style={{ background: "linear-gradient(135deg, #0d2818 0%, #111 100%)", border: "1px solid #1a3a2a", borderRadius: 16, padding: "32px", marginBottom: 48, textAlign: "center" }}>
          <div style={{ fontSize: 24, fontWeight: 800, color: "#fff", marginBottom: 8 }}>Stop tracking bonuses in a spreadsheet</div>
          ...
          <Link href="/" style={{
            ...
            background: "#88e06d", color: "#000", borderRadius: 10, textDecoration: "none",
          }}>
            See your projected earnings &rarr;
          </Link>
          <div style={{ fontSize: 12, color: "#555", marginTop: 12 }}>$5/month or $50/year. Most first bonuses are $300-$400.</div>
        </div>

        {/* ── FAQ ── */}
        <div id="faq" style={{ marginBottom: 48 }}>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#fff", margin: "0 0 20px" }}>Frequently Asked Questions</h2>
          {[
            ...
            <div key={i} style={{ borderBottom: "1px solid #1a1a1a", padding: "16px 0" }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: "#fff", margin: "0 0 8px" }}>{faq.q}</h3>
        ...
        {/* YouTube CTA */}
        <div style={{ padding: "24px", background: "rgba(255,0,0,0.05)", border: "1px solid rgba(255,0,0,0.15)", borderRadius: 12, marginBottom: 40 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", marginBottom: 8 }}>Watch: Bank Bonus Strategies on YouTube</div>
        ...
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
          ...
          <Link href="/blog/best-checking-bonuses-2026" style={{ fontSize: 14, color: "#88e06d", textDecoration: "none", fontWeight: 600 }}>Best checking bonuses</Link>
          <Link href="/blog/best-savings-bonuses-2026" style={{ fontSize: 14, color: "#88e06d", textDecoration: "none", fontWeight: 600 }}>Best savings bonuses</Link>
        </div>
      </main>
