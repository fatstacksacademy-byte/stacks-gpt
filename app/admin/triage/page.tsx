...
// Friendly labels keyed off the dotted catalog path. Falls back to title-casing
// the leaf segment so unfamiliar fields still render decently.
const FIELD_LABELS: Record<string, string> = {
  "bonus_amount": "Bonus Amount",
  "expired": "Expired Flag",
  "requirements.min_direct_deposit_total": "Min Direct Deposit Total",
  "requirements.deposit_window_days": "Deposit Window (days)",
  "fees.monthly_fee": "Monthly Fee",
}
...
// Render any JSON-ish value as a short, readable string. Numbers get a
// dollar sign for the money fields; booleans get a yes/no; null is "—".
function renderValue(path: string, value: unknown): string {
  if (value === null || value === undefined) return "—"
  if (typeof value === "boolean") return value ? "Yes" : "No"
  if (typeof value === "number") {
    if (path.includes("amount") || path.includes("deposit_total") || path.includes("fee")) {
...
export default function TriagePage() {
  ...
  const [onlyDisagrees, setOnlyDisagrees] = useState(false)
  ...
  const load = useCallback(async () => {
    setLoading(true)
    setError(null)
    const res = await fetch("/api/admin?action=triage-queue")
    if (res.ok) {
  ...
  const loadOverrides = useCallback(async () => {
    const res = await fetch("/api/admin?action=bonus-url-overrides")
    if (res.ok) {
      const data = await res.json()
  ...
  const filteredQueue = useMemo(() => {
    return queue.filter((q) => {
      if (search && !q.bank_name.toLowerCase().includes(search.toLowerCase())) return false
      if (fieldFilter !== "all" && q.field_path !== fieldFilter) return false
      if (onlyDisagrees && q.consensus.agrees !== false) return false
      return true
    })
  }, [queue, search, fieldFilter, onlyDisagrees])

  const remaining = useMemo(
    () => filteredQueue.filter((q) => !decidedKeys.has(`${q.bonus_id}::${q.field_path}`)),
  ...
  const currentKey = current ? `${current.bonus_id}::${current.field_path}` : null
  ...
  const advance = useCallback(() => {
    ...
    for (let step = 1; step <= filteredQueue.length; step++) {
      ...
      const k = `${filteredQueue[idx].bonus_id}::${filteredQueue[idx].field_path}`
  ...
  const decide = useCallback(
    async (verdict: Verdict) => {
      ...
      setBusy(true)
      setError(null)
      const res = await fetch("/api/admin?action=triage-decide", {
        method: "POST",
  ...
  const submitFlagReport = useCallback(
    async (input: { issue_category: string; issue_description: string; suggested_fix: string }) => {
      ...
      setBusy(true)
      setError(null)
      const res = await fetch("/api/admin?action=report-flag-issue", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bonus_id: current.bonus_id,
  ...
  const submitUrlOverride = useCallback(
    async (override_url: string, discovery_method: string) => {
      ...
      setBusy(true)
      setError(null)
      const overrideRes = await fetch("/api/admin?action=bonus-url-override", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bonus_id: current.bonus_id,
      ...
      await fetch("/api/admin?action=triage-decide", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bonus_id: current.bonus_id,
    ...
    <div style={{ minHeight: "100vh", background: "#fafafa", fontFamily: "-apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      <div style={{ borderBottom: "1px solid #e8e8e8", background: "#fff", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: "#0d7c5f", background: "#e6f5f0", padding: "2px 8px", borderRadius: 99 }}>TRIAGE</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          {filterOpen && (
            <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <label style={{ fontSize: 13, color: "#555", display: "flex", alignItems: "center", gap: 8 }}>
                <input
                  type="checkbox"
                  checked={onlyDisagrees}
                  onChange={(e) => setOnlyDisagrees(e.target.checked)}
                />
                Only show consensus disagrees
              </label>
            </div>
          )}
        </div>
          ...
          {recent.map((o) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <span style={{ fontFamily: "monospace", color: "#7c3aed", fontWeight: 700 }}>{o.bonus_id}</span>
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 12, padding: "32px 20px", textAlign: "center" }}>
      ...
      <code style={{ display: "inline-block", background: "#f5f5f5", border: "1px solid #e8e8e8", borderRadius: 6, padding: "6px 10px", fontSize: 12, color: "#111" }}>
        npx tsx scripts/verify-bonuses/run.ts --persist
      </code>
    </div>
  )
}

function ConsensusBadge({ consensus }: { consensus: QueueEntry["consensus"] }) {
  if (consensus.agrees === true) {
    return (
      <span style={{ fontSize: 11, fontWeight: 700, color: "#0d7c5f", background: "#e6f5f0", padding: "2px 8px", borderRadius: 99, display: "inline-flex", alignItems: "center", gap: 4 }}>
        <span aria-hidden>✓</span> DoC agrees
      </span>
    )
  }
  if (consensus.agrees === false) {
    return (
      <span style={{ fontSize: 11, fontWeight: 700, color: "#b91c1c", background: "#fee2e2", padding: "2px 8px", borderRadius: 99, display: "inline-flex", alignItems: "center", gap: 4 }}>
        <span aria-hidden>✕</span> DoC disagrees
      </span>
    )
  }
  return (
    <span style={{ fontSize: 11, fontWeight: 700, color: "#666", background: "#f5f5f5", padding: "2px 8px", borderRadius: 99, display: "inline-flex", alignItems: "center", gap: 4 }}>
      <span aria-hidden>—</span> single source
    </span>
  )
}
...
function TriageCard({
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* Bonus identity */}
      <div>
        <div style={{ fontSize: 18, fontWeight: 700, color: "#111", lineHeight: 1.2 }}>{entry.bank_name}</div>
        <div style={{ fontSize: 11, color: "#bbb", marginTop: 4, fontFamily: "monospace" }}>{entry.bonus_id}</div>
      ...
      {/* Consensus + page signal */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <ConsensusBadge consensus={entry.consensus} />
        {entry.consensus.secondary_url && (
          <a href={entry.consensus.secondary_url} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: "#2563eb" }}>
            secondary source
          </a>
        )}
        ...
        {entry.consensus.confidence && (
          <span style={{ fontSize: 11, color: "#666", background: "#f5f5f5", padding: "2px 8px", borderRadius: 99 }}>
            confidence: {entry.consensus.confidence}
          </span>
        )}
      </div>
